	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'storeys'          => $query->row['storeys'],
				'furniture'        => $query->row['furniture'],
				'numrooms'         => $query->row['numrooms'],
				'remont'           => $query->row['remont'],
				'pricerus'           => $query->row['pricerus'],
				'phone'            => $query->row['phone'],
				'FIO'         	   => $query->row['FIO'],
				'purpose'         	   => $query->row['purpose'],
				'ownership'         	   => $query->row['ownership'],
				'Light'         	   => $query->row['Light'],
				'Gas'         	   => $query->row['Gas'],
				'Water'         	   => $query->row['Water'],
				'sewerage'         	   => $query->row['sewerage'],
				'colnums'        => $query->row['colnums'],
				'ucharea'        => $query->row['ucharea'],
				'colsanuz'        => $query->row['colsanuz'],
				'ymore'        	   => $query->row['ymore'],
				'vidmore'          => $query->row['vidmore'],
				'vidgori'          => $query->row['vidgori'],
				'komunikacion'     => $query->row['komunikacion'],
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				$sql .= " FROM " . DB_PREFIX . "category_path cp INNER JOIN " . DB_PREFIX . "product_to_category p2c ON (cp.category_id = p2c.category_id)";
			} else {
				$sql .= " FROM " . DB_PREFIX . "product_to_category p2c";
			}

			if (!empty($data['filter_filter'])) {
				$sql .= " INNER  JOIN " . DB_PREFIX . "product_filter pf ON (p2c.product_id = pf.product_id) INNER  JOIN " . DB_PREFIX . "product p ON (pf.product_id = p.product_id)";
			} else {
